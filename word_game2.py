...
def get_guess():
	...
	if not guess == "":               ### If the input is empty, it returns guess...user is asked to enter an input
		...
		return get_guess()                     ###recursion takes place here....it repeats itself
...
def evaluate_guess(word, attempts):
	while attempts > 0:
		...
		if guess != word:
			attempts -= 1
			print("Wrong attempt, you have ", attempts, "attempt(s) left")
			return evaluate_guess(word, attempts)
		else:
...
def play_game(game_count):
	...
	while evaluate_guess(word, 5):
		...
		if  ask == "y":
			game_count += 1
			print("You played ", game_count, "game(s)")
